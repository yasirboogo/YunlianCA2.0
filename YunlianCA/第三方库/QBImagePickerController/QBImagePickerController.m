...
- (void)done:(id)sender
{
    [self passSelectedAssetsToDelegateWith:NO];
}
...
- (void)passSelectedAssetsToDelegateWith:(BOOL)IsChoose
{
        ...
        [self.assetsLibrary assetForURL:selectedAssetURL
                            resultBlock:^(ALAsset *asset) {
                                ...
                                if (assets.count == weakSelf.selectedAssetURLs.count) {
                                    // Delegate
                                    if (self.delegate && [self.delegate respondsToSelector:@selector(imagePickerController:didSelectAssets:withChoose:)]) {
                                        [self.delegate imagePickerController:self didSelectAssets:[assets copy]withChoose:IsChoose];
                                    }
                                }
                            } failureBlock:^(NSError *error) {
                                NSLog(@"Error: %@", [error localizedDescription]);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    assetsCollectionViewController.isOneImage=self.isOneImage;
    assetsCollectionViewController.isInsertText=self.isInsertText;
...
- (void)assetsCollectionViewControllerDidFinishSelection:(QBAssetsCollectionViewController *)assetsCollectionViewController withChoose:(BOOL)isChoose
{
    [self passSelectedAssetsToDelegateWith:isChoose];
}
