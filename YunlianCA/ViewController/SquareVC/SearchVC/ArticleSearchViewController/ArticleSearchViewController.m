...
@interface ArticleSearchViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.historyArray=[[NSMutableArray alloc]initWithArray:[defaults arrayForKey:@"articleHistory"]];
...
-(void)searchBtnClick
{
    ...
    else
    {
        ...
        [hud show:YES];
        [HTTPManager getArticleListWithUserId:[UserInfoData getUserInfoFromArchive].userId moduleId:self.moduleId pxType:@"qwzr" pageNum:@"1" pageSize:@"100" areaId:[UserInfoData getUserInfoFromArchive].areaId keyword:keyword complete:^(Model *model) {
            if ([model.result isEqualToString:STATUS_SUCCESS])
            {
                [hud hide:YES];
                NSLog(@"搜索结果===%@",model.articleListArray);
                
                if (model.articleListArray.count>0)
                {
                    //有搜索结果 跳转搜索结果界面
                    ArticleSearchResultsViewController *vc=[[ArticleSearchResultsViewController alloc]init];
                    vc.resultsArray=model.articleListArray;
                ...
                NSUserDefaults *defaults=[NSUserDefaults standardUserDefaults];
                NSMutableArray *historyArray=[[NSMutableArray alloc]initWithArray:[defaults arrayForKey:@"articleHistory"]];
                if (historyArray==nil)
                {
                ...
                [defaults setObject:historyArr forKey:@"articleHistory"];
                [defaults synchronize];
                
                //取出本地历史记录
                NSUserDefaults *defa=[NSUserDefaults standardUserDefaults];
                self.historyArray=[[NSMutableArray alloc]initWithArray:[defa arrayForKey:@"articleHistory"]];
                [_historyTableView reloadData];
            }
            else
            {
...
-(void)clearBtnClick
{
    ...
    //清空数据
    NSUserDefaults *defaults=[NSUserDefaults standardUserDefaults];
    [defaults setObject:[[NSArray alloc]init] forKey:@"articleHistory"];
