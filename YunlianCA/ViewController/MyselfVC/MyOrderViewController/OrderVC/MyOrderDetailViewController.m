...
-(void)getOrderList
{
    ...
    [HTTPManager getOrderListOfUserWithUserId:[UserInfoData getUserInfoFromArchive].userId status:self.orderStatus pageNum:@"1" pageSize:@"20" complete:^(OrderModel *model) {
...
-(void)getMoreData
{
    ...
    [HTTPManager getOrderListOfUserWithUserId:[UserInfoData getUserInfoFromArchive].userId status:self.orderStatus pageNum:[NSString stringWithFormat:@"%ld",self.pageNum] pageSize:@"20" complete:^(OrderModel *model) {
        if ([model.result isEqualToString:STATUS_SUCCESS])
        {
            for (NSDictionary *orderDict in model.orderListArray)
            {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (isOnline==0)
    {
        if ([self.orderStatus isEqualToString:@"4"] || [self.orderStatus isEqualToString:@"5"])
        {
            //如果是退款订单 跳转到退款情况界面
...
#warning 判断余额是否足够
        ...
        [HTTPManager payOrderWithPayInfoId:self.payOrderId ymlPayPass:self.passwordTF.text complete:^(NSDictionary *resultDict) {
            ...
            if ([result isEqualToString:STATUS_SUCCESS])
            {
                ...
                [_tableView reloadData];
                [self.view makeToast:@"订单支付成功" duration:2.0];
            }
            else
            {
